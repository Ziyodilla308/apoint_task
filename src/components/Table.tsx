...
function TreeTable({ groupedData }: { groupedData: unknown }) {
    ...
    if (typeof groupedData !== "object" || groupedData === null) {
        return <div className="p-6 text-red-500">Invalid data</div>;
    }

    const allItems: any[] = Object.values(groupedData).flatMap((cat) => {
        if (typeof cat !== "object" || cat === null) return [];
        return Object.values(cat).flatMap((sub) =>
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                ...
                                                                {keys.map(
                                                                    (k) => (
                                                                        <td
                                                                            key={`cat-${categoryKey}-${k}`}
                                                                            className="text-right px-4 py-3 text-gray-600"
                                                                        >
                                                                            {formatNumber(
                                                                                categoryTotals[
                                                                                    k
                                                            ...
                                                            {isCategoryOpen &&
                                                                safeItems.map(
                                                                    (
                                                                        ...
                                                                        <tr
                                                                            key={`item-${item?.material_id ?? idx}`}
                                                                            className="border-t hover:bg-gray-50"
                                                                        >
                                                                            <td className="pl-16 py-2 text-gray-800">
                                                                                {item?.name ??
                                                                                    "-"}
                                                                            </td>
                                                                            {keys.map(
                                                                                (
                                                                                    ...
                                                                                    <td
                                                                                        key={`item-${item?.id ?? idx}-${k}`}
                                                                                        className="text-right px-4 py-2 text-gray-700"
                                                                                    >
                                                                                        {formatNumber(
                                                                                            Number(
                                                                                                item?.[
