...
function TreeTable({ groupedData }: { groupedData: any }) {
            ...
            <div className="border border-gray-200 rounded-xl shadow-sm bg-white">
                <table className="min-w-full text-sm text-gray-800">
                    ...
                    <tr>
                        ...
                        {keys.map(k => (
                            <th key={k} className="px-4 py-3 text-right capitalize">{k.replaceAll('_', ' ')}</th>
                        ))}
                    </tr>
                    </thead>
                    <tbody>
                                ...
                                {isParentOpen && Object.entries(categories).map(([category, items]) => {
                                    ...
                                    return (
                                        <Fragment key={`category-${categoryKey}`}>
                                            ...
                                            {isCategoryOpen && items.map(item => (
                                                <tr key={`item-${item.material_id}`} className="border-t hover:bg-gray-50">
                                                    <td className="pl-16 py-2 text-gray-800">{item.name}</td>
                                                    {keys.map(k => (
                                                        <td key={`item-${item.id}-${k}`} className="text-right px-4 py-2 text-gray-700">
                                                            {formatNumber(Number(item[k]))}
                                                        </td>
                                                    ))}
                                                </tr>
                                            ))}
